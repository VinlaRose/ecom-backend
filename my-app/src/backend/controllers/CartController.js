...
export const removeItemFromCartHandler = function (schema, request) {
  ...
  try {
    ...
    userCart = userCart.filter((item) => item.cartProduct.item_id !== Number(productId));
    this.db.users.update({ _id: userId }, { cart: userCart });
    return new Response(200, {}, { cart: userCart });
  } catch (error) {
    return new Response(
...
export const updateCartItemHandler = function (schema, request) {
  ...
  try {
    ...
    const { action } = request.requestBody;
    console.log(action)
    ...
    if (action.type === "increment") {
      ...
      userCart.forEach((product) => {
        ...
        if (product.cartProduct.item_id === Number(productId)) {
          product.qty += 1;
